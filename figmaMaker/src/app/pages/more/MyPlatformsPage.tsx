...
import {
  ArrowLeft, Users, ChevronRight, LayoutGrid, MapPin, Clock,
} from 'lucide-react';
...
export function MyPlatformsPage() {
  ...
  const renderPlatformCard = (p: typeof platforms[0], isProposed = false) => {
        ...
        <div className="flex gap-3 p-4">
          ...
          <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center shrink-0 overflow-hidden">
            {p.posterUrl ? (
              ...
              <LayoutGrid size={20} strokeWidth={1} className="text-blue-200" />
            )}
          </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2 mb-1.5">
              <p className="font-bold text-gray-800 text-sm line-clamp-1 flex-1">{p.title}</p>
              <button
            ...
            {/* Status Badges + Participant count */}
            <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
              {p.lifecycle === 'pending' ? (
                ...
                activeStates.filter(state => activeStateMeta[state]).map((state) => (
              ...
              <div className="flex items-center gap-1 text-xs text-gray-400 ml-auto">
                <Users size={11} />
                <span>{(p.participants || []).length + 1}명</span>
              </div>
            </div>
            ...
            {/* State Toggles — only for proposed platforms that are active */}
            {isProposed && p.lifecycle === 'active' && (
              <div className="mt-2 pt-2 border-t border-gray-100">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">플랫폼 상태 변경</p>
  ...
  return (
    <div className="flex flex-col min-h-full bg-gray-50">
      ...
      <div className="flex-1 px-4 py-5 space-y-5">
        {/* My Proposed */}
        <div>
          <p className="text-xs font-bold uppercase tracking-wider mb-3 text-[#505c71] text-[#606a79]">내가 제안한 플랫폼 ({myProposed.length})</p>
          {myProposed.length === 0 ? (
        ...
        {/* My Participating */}
        <div>
          <p className="text-xs font-bold uppercase tracking-wider mb-3 text-[#606a79]">내가 참여한 플랫폼 ({myParticipating.length})</p>
