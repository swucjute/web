...
function Field({ label, icon, children, locked }: FieldProps) {
  return (
    <div>
      <label className="flex items-center gap-1.5 text-xs font-semibold text-gray-500 mb-1.5">
        ...
        <span>{label}</span>
        {locked && (
          <span className="ml-auto flex items-center gap-0.5 text-[10px] text-gray-400 font-normal">
            <Lock size={10} /> 관리자만 수정
          </span>
        )}
      </label>
      {children}
    ...
    <div className="flex flex-col min-h-full bg-gray-50">
      ...
      <div className="bg-white sticky top-0 z-20 border-b border-gray-100">
        <div className="flex items-center gap-3 px-4 h-14">
          <button onClick={() => navigate(-1)} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 active:bg-gray-200 transition-colors">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="bg-white flex flex-col items-center py-7 border-b border-gray-100">
          ...
          <div className="flex items-center gap-2">
            <span className="font-bold text-gray-900 text-lg">{currentUser.name}</span>
            <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${roleColor(currentUser.role)}`}>
              {roleLabel(currentUser.role)}
            </span>
          </div>
          <p className="text-xs text-gray-400 mt-1">가입일 {currentUser.joinDate?.replace(/-/g, '.')}</p>
        </div>
          ...
          <div>
            ...
            <div className="bg-white rounded-2xl p-4 space-y-4 shadow-sm">
              ...
              <Field label="권한" icon={<Shield size={12} />}>
                <div className={`${inputDisabled} flex items-center gap-2`}>
                  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${roleColor(currentUser.role)}`}>
                    {roleLabel(currentUser.role)}
                  </span>
                </div>
              </Field>
          ...
          <div>
            ...
            <div className="bg-white rounded-2xl p-4 space-y-4 shadow-sm">
              ...
              <Field label="생년월일" icon={<Cake size={12} />}>
                <input type="date" value={form.birthDate} onChange={(e) => setForm({ ...form, birthDate: e.target.value })} className={inputNormal} />
              </Field>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="bg-white rounded-2xl p-4 space-y-4 shadow-sm">
              ...
              <Field label="계좌번호" icon={<CreditCard size={12} />}>
                <input type="text" value={form.accountNumber} onChange={(e) => { const val = e.target.value.replace(/[^\d-]/g, ''); setForm({ ...form, accountNumber: val }); }} placeholder="계좌번호 입력 (숫자만)" inputMode="numeric" className={inputNormal} />
              </Field>
